...
fit=readRDS("fit_2021_1_15Nov.rds")
# original fit
fit$pop_adjust <- TRUE
fit_orig <- fit
...
p1 <-ggplot(dfrt,aes(x = Date, y = Value,color=Ind)
  ) + geom_line(size=1) +
...
p1 <- p1 + ggplot2::labs(y = "Median Rt") + ggplot2::facet_wrap(~Group,scales="free_y") + 
  ggtitle("Effective R trajectory Jan 1- June 30 2021") + theme(legend.title=element_text(size=12))+
 theme(legend.text=element_text(size=12)) + xlab("")



ggsave("rt2021one.pdf",p1,width=11,height=8)
...
p1 <- base +
  ...
  )  + xlim(as.Date('2021-01-01'), as.Date('2021-06-30')) +  scale_x_date(
  date_breaks = date_breaks,
  labels = scales::date_format("%b")
)
p1 <- p1 + ggplot2::labs(y = "Deaths") + ggplot2::facet_wrap(~group,scales="free_y") +
  scale_color_brewer(palette="Paired") + ggtitle("Deaths Jan 1- June 30 2021")+xlab("")
ggsave("death2021one.pdf",p1,width=11,height=8)
...
# ggsave("scenario-death_Bang_1.pdf",p2,width=9,height=6)
# 
# p3 <- base +
#   ggplot2::geom_line(
#     mapping = ggplot2::aes(x = date, y = deaths_median, color = colour),
...
# ggsave("scenario-death_Pak_1.pdf",p3,width=9,height=6)
# 
# 
# p4 <- base +
#   ggplot2::geom_line(
...
# ggsave("scenario-death_Nep_1.pdf",p4,width=9,height=6)
# 
# p5 <- base +
#   ggplot2::geom_line(
#     mapping = ggplot2::aes(x = date, y = deaths_median, color = colour),
...
# ggsave("scenario-death_Sri_1.pdf",p5,width=9,height=6)
# 
# 
# margin = theme(plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"))
# p1=p1+margin
# ggsave("scenario-deaths_Ind_1.pdf",p1,width=11,height=8)
# p2=p2+margin
# ggsave("scenario-deaths_Bang_1.pdf",p2,width=11,height=8)
# p3=p3+margin
# ggsave("scenario-deaths_Pak_1.pdf",p3,width=11,height=8)
# p4=p4+margin
# ggsave("scenario-deaths_Nep_1.pdf",p4,width=11,height=8)
# p5=p5+margin
# ggsave("scenario-deaths_Sri_1.pdf",p5,width=11,height=8)
