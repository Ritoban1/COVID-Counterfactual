...
args$sampling_args <- list(iter=1500, seed=12345, control=list(adapt_delta=0.95,max_treedepth=15))
...
#saveRDS(fit,"fit_2020_uc.rds")

fit=readRDS("fit_2020_uc.rds")
# original fit
fit$pop_adjust <- TRUE
fit_orig <- fit
...
# df_b_m=cumsum(df[(df$group=="Nepal" & df$date>=changeDate),]$deaths_median)/29.14
# df5_b_m=cumsum(df_cf5[(df_cf5$group=="Nepal" & df_cf5$date>=changeDate),]$deaths_median)/29.14
# df6_b_m=cumsum(df_cf6[(df_cf6$group=="Nepal" & df_cf6$date>=changeDate),]$deaths_median)/29.14
# df7_b_m=cumsum(df_cf7[(df_cf8$group=="Nepal" & df_cf7$date>=changeDate),]$deaths_median)/29.14
# df8_b_m=cumsum(df_cf8[(df_cf8$group=="Nepal" & df_cf8$date>=changeDate),]$deaths_median)/29.14
# 
# df_b_l=cumsum(df[(df$group=="Nepal" & df$date>=changeDate),]$deaths_li)/29.14
# df5_b_l=cumsum(df_cf5[(df_cf5$group=="Nepal" & df_cf5$date>=changeDate),]$deaths_li)/29.14
# df6_b_l=cumsum(df_cf6[(df_cf6$group=="Nepal" & df_cf6$date>=changeDate),]$deaths_li)/29.14
# df7_b_l=cumsum(df_cf7[(df_cf8$group=="Nepal" & df_cf7$date>=changeDate),]$deaths_li)/29.14
# df8_b_l=cumsum(df_cf8[(df_cf8$group=="Nepal" & df_cf8$date>=changeDate),]$deaths_li)/29.14
# 
# df_b_u=cumsum(df[(df$group=="Nepal" & df$date>=changeDate),]$deaths_ui)/29.14
# df5_b_u=cumsum(df_cf5[(df_cf5$group=="Nepal" & df_cf5$date>=changeDate),]$deaths_ui)/29.14
# df6_b_u=cumsum(df_cf6[(df_cf6$group=="Nepal" & df_cf6$date>=changeDate),]$deaths_ui)/29.14
# df7_b_u=cumsum(df_cf7[(df_cf8$group=="Nepal" & df_cf7$date>=changeDate),]$deaths_ui)/29.14
# df8_b_u=cumsum(df_cf8[(df_cf8$group=="Nepal" & df_cf8$date>=changeDate),]$deaths_ui)/29.14
# 
# 
# 
# tail(df_b_m,1)
...
dfsub=df[-which(df$group %in% unique(df$group)[c(6:25)]),]
...
dfsub$group=factor(dfsub$group,levels=c("Fit Ind","Rel: Ind -> Bang","Rel: Ind -> Pak","Rel: Ind -> Sri","Rel: Ind -> Nep",
                                        "Rel: Bang -> Ind","Fit Bang","Rel: Bang -> Pak", "Rel: Bang -> Sri","Rel: Bang -> Nep",
                                        "Rel: Pak -> Ind", "Rel: Pak -> Bang", "Fit Pak", "Rel: Pak -> Sri", "Rel: Pak -> Nep",
                                        "Rel: Sri -> Ind", "Rel: Sri -> Bang", "Rel: Sri -> Pak", "Fit Sri", "Rel: Sri -> Nep",
                                        "Rel: Nep -> Ind", "Rel: Nep -> Bang", "Rel: Nep -> Pak", "Rel: Nep -> Sri","Fit Nep"))
dfsub$group=recode_factor(dfsub$group,"Fit Ind"="Fit India","Rel: Ind -> Bang"="India -> Bangladesh",
                          "Rel: Ind -> Pak"="India -> Pakistan",
                          "Rel: Ind -> Sri"="India -> Sri Lanka","Rel: Ind -> Nep"="India -> Nepal",
                          "Rel: Bang -> Ind"="Bangladesh -> India","Fit Bang"="Fit Bangladesh",
                          "Rel: Bang -> Pak"="Bangladesh -> Pakistan", "Rel: Bang -> Sri"="Bangladesh -> Sri Lanka",
                          "Rel: Bang -> Nep"="Bangladesh -> Nepal", "Rel: Pak -> Ind"="Pakistan -> India", 
                          "Rel: Pak -> Bang"="Pakistan -> Bangladesh", 
                          "Fit Pak"="Fit Pakistan",
                          "Rel: Pak -> Sri"="Pakistan -> Sri Lanka", "Rel: Pak -> Nep"="Pakistan -> Nepal",
                          "Rel: Sri -> Ind"="Sri Lanka -> India", "Rel: Sri -> Bang"="Sri Lanka -> Bangladesh", 
                          "Rel: Sri -> Pak"="Sri Lanka -> Pakistan",
                          "Fit Sri"="Fit Sri Lanka",
                          "Rel: Sri -> Nep"="Sri Lanka -> Nepal",
                          "Rel: Nep -> Ind"="Nepal -> India", 
                          "Rel: Nep -> Bang"="Nepal -> Bangladesh", 
                          "Rel: Nep -> Pak"="Nepal -> Pakistan", "Rel: Nep -> Sri"="Nepal -> Sri Lanka",
                          "Fit Nep"="Fit Nepal")

x=dfsub$median[which(dfsub$group%in%c("Fit Bangladesh","Fit India","Fit Nepal","Fit Pakistan","Fit Sri Lanka"))]
dfsub$rtfit=rep(x,times=5)
dfrt1=data.frame("Group"=dfsub$group,"Value"=dfsub$median,"Ind"=rep("Counterfactual",times=length(dfsub$median)),"Date"=dfsub$date)
dfrt1=dfrt1[-which(dfrt1$Group %in%c("Fit India","Fit Bangladesh","Fit Pakistan","Fit Sri Lanka","Fit Nepal")),]
dfrt2=data.frame("Group"=dfsub$group,"Value"=dfsub$rtfit,"Ind"=rep("Actual",times=length(dfsub$rtfit)),"Date"=dfsub$date)
...
p1 <-ggplot(dfrt,aes(x = Date, y = Value,color=Ind)) + geom_line(size=1) +
...
ggsave("rt2020two.pdf",p1,width=13,height=8)
...
ggsave("death2020two.pdf",p1,width=13,height=8)
...
# 
# ggsave("scenario-rt_Ind_20.pdf",p1,width=9,height=6)
# 
# p2 <- base +
#   ggplot2::geom_line(
#     mapping = ggplot2::aes(x = date, y = median, color = colour),
...
#     size = 1,color='black') + xlim(as.Date('2020-03-15'), as.Date('2020-12-31'))
...
# ggsave("scenario-rt_Bang_20.pdf",p2,width=9,height=6)
# 
# 
# p3 <- base +
#   ggplot2::geom_line(
...
# ggsave("scenario-rt_Pak_20.pdf",p3,width=9,height=6)
# 
# 
# p4 <- base +
#   ggplot2::geom_line(
...
# ggsave("scenario-rt_nep_20.pdf",p4,width=9,height=6)
# 
# 
# 
# p5 <- base +
...
#     size = 1,color='black'
#   ) + xlim(as.Date('2020-03-15'), as.Date('2020-12-31'))
# p5 <- p5 + ggplot2::labs(y = "Median Rt") + ggplot2::facet_wrap(~group,,scales="free_y") + scale_color_brewer(palette="Paired")  + ggtitle("Effective R trajectory for Sri Lanka (Recipient)")
# ggsave("scenario-rt_sri_20.pdf",p5,width=9,height=6)
# 
# 
# 
# 
...
# ggsave("scenario-death_Ind_20.pdf",p1,width=9,height=6)
# 
# p2 <- base +
#   ggplot2::geom_line(
#     mapping = ggplot2::aes(x = date, y = deaths_median, color = colour),
...
# ggsave("scenario-death_Bang_20.pdf",p2,width=9,height=6)
# 
# p3 <- base +
#   ggplot2::geom_line(
#     mapping = ggplot2::aes(x = date, y = deaths_median, color = colour),
...
# ggsave("scenario-death_Pak_20.pdf",p3,width=9,height=6)
# 
# 
# p4 <- base +
#   ggplot2::geom_line(
...
# ggsave("scenario-death_Nep_20.pdf",p4,width=9,height=6)
# 
# p5 <- base +
#   ggplot2::geom_line(
#     mapping = ggplot2::aes(x = date, y = deaths_median, color = colour),
...
#     width = 0.5,fill='royalblue4',alpha=0.7,
#   ) + xlim(as.Date('2020-03-15'), as.Date('2020-12-31'))
# p5<- p5 + ggplot2::labs(y = "Deaths") + ggplot2::facet_wrap(~group,scales="free_y") + scale_color_brewer(palette="Paired") + ggtitle("Sri Lanka")
# ggsave("scenario-death_Sri_20.pdf",p5,width=9,height=6)
# 
# 
# margin = theme(plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"))
# p1=p1+margin
...
# ######################################################################################################################################################
# # colours thanks for Michael Betancourts aesthetics
# UFR=readRDS("URF_2020.rds")
# UFR_init=29.1
# 
# x=seq(as.Date("2020-03-15"), as.Date("2020-12-31"), by = 1)
# 
# tot_cases_14320_ind=data_total_cases[data_total_cases$Date==as.Date("2020-03-14"),"India"]
# tot_cases_14320_bang=data_total_cases[data_total_cases$Date==as.Date("2020-03-14"),"Bangladesh"]
# tot_cases_14320_pak=data_total_cases[data_total_cases$Date==as.Date("2020-03-14"),"Pakistan"]
# tot_cases_14320_sri=data_total_cases[data_total_cases$Date==as.Date("2020-03-14"),"Sri_Lanka"]
# tot_cases_14320_nep=data_total_cases[data_total_cases$Date==as.Date("2020-04-14"),"Nepal"]
# ci <- c("#C79999")
# mn <- c("#7C0000")
# data_cases=data_cases[data_cases$Date>=changeDate,]
# data_india=rep(cumsum(data_cases$India),9)+tot_cases_14320_ind
# data_bang=rep(cumsum(data_cases$Bangladesh),9)+tot_cases_14320_bang
# data_pak=rep(cumsum(data_cases$Pakistan),9)+tot_cases_14320_pak
# data_nep=rep(cumsum(data_cases$Nepal),9)+tot_cases_14320_nep
# data_sri=rep(cumsum(data_cases$Sri_Lanka),9)+tot_cases_14320_sri
# 
# tot_tot_cases_14320_ind=(data_total_cases[data_total_cases$Date==as.Date("2020-03-14"),"India"])*UFR_init
# tot_tot_cases_14320_bang=data_total_cases[data_total_cases$Date==as.Date("2020-03-14"),"Bangladesh"]*UFR_init
# tot_tot_cases_14320_pak=data_total_cases[data_total_cases$Date==as.Date("2020-03-14"),"Pakistan"]*UFR_init
# tot_tot_cases_14320_sri=data_total_cases[data_total_cases$Date==as.Date("2020-03-14"),"Sri_Lanka"]*UFR_init
# tot_tot_cases_14320_nep=data_total_cases[data_total_cases$Date==as.Date("2020-03-14"),"Nepal"]*UFR_init
# 
# dim_ind=nrow(df[df$country%in%c("India"),])
# dim_bang=nrow(df[df$country%in%c("Bangladesh"),])
# dim_pak=nrow(df[df$country%in%c("Pakistan"),])
...
# 
# 
# UUFR=UFR/((tot_tot_cases_14320_ind+cumsum(df[df$country%in%c("India"), ]$infections_median[1:(dim_ind/9)]))/data_india[1:(dim_ind/9)])
# UUFR=rep(UUFR,9)
# 
# 
# 
# 
# 
# df[df$country%in%c("India"),]$UFR_med=(tot_tot_cases_14320_ind+
#                                          c(cumsum(df[df$country%in%c("India"),]$infections_median[1:(dim_ind/9)]),
#                                            cumsum(df[df$country%in%c("India"),]$infections_median[(dim_ind/9+1):(2*dim_ind/9)]),
#                                            cumsum(df[df$country%in%c("India"),]$infections_median[(2*dim_ind/9+1):(3*dim_ind/9)]),
#                                            cumsum(df[df$country%in%c("India"),]$infections_median[(3*dim_ind/9+1):(4*dim_ind/9)]),
...
# 
# df[df$country%in%c("Bangladesh"),]$UFR_med=(tot_tot_cases_14320_bang+
#                                               c(cumsum(df[df$country%in%c("Bangladesh"),]$infections_median[1:(dim_bang/9)]),
#                                                 cumsum(df[df$country%in%c("Bangladesh"),]$infections_median[(dim_bang/9+1):(2*dim_bang/9)]),
#                                                 cumsum(df[df$country%in%c("Bangladesh"),]$infections_median[(2*dim_bang/9+1):(3*dim_bang/9)]),
#                                                 cumsum(df[df$country%in%c("Bangladesh"),]$infections_median[(3*dim_bang/9+1):(4*dim_bang/9)]),
...
# 
# df[df$country%in%c("Pakistan"),]$UFR_med=(tot_tot_cases_14320_pak+
#                                             c(cumsum(df[df$country%in%c("Pakistan"),]$infections_median[1:(dim_pak/9)]),
#                                               cumsum(df[df$country%in%c("Pakistan"),]$infections_median[(dim_pak/9+1):(2*dim_pak/9)]),
#                                               cumsum(df[df$country%in%c("Pakistan"),]$infections_median[(2*dim_pak/9+1):(3*dim_pak/9)]),
#                                               cumsum(df[df$country%in%c("Pakistan"),]$infections_median[(3*dim_pak/9+1):(4*dim_pak/9)]),
...
# 
# df[df$country%in%c("India"),]$UFR_li=(tot_tot_cases_14320_pak+
#                                         c(cumsum(df[df$country%in%c("India"),]$infections_li[1:(dim_ind/9)]),
#                                           cumsum(df[df$country%in%c("India"),]$infections_li[(dim_ind/9+1):(2*dim_ind/9)]),
#                                           cumsum(df[df$country%in%c("India"),]$infections_li[(2*dim_ind/9+1):(3*dim_ind/9)]),
#                                           cumsum(df[df$country%in%c("India"),]$infections_li[(3*dim_ind/9+1):(4*dim_ind/9)]),
...
# 
# df[df$country%in%c("Bangladesh"),]$UFR_li=(tot_tot_cases_14320_bang+
#                                              c(cumsum(df[df$country%in%c("Bangladesh"),]$infections_li[1:(dim_bang/9)]),
#                                                cumsum(df[df$country%in%c("Bangladesh"),]$infections_li[(dim_bang/9+1):(2*dim_bang/9)]),
#                                                cumsum(df[df$country%in%c("Bangladesh"),]$infections_li[(2*dim_bang/9+1):(3*dim_bang/9)]),
#                                                cumsum(df[df$country%in%c("Bangladesh"),]$infections_li[(3*dim_bang/9+1):(4*dim_bang/9)]),
...
# 
# 
# df[df$country%in%c("Pakistan"),]$UFR_li=(tot_tot_cases_14320_pak+
#                                            c(cumsum(df[df$country%in%c("Pakistan"),]$infections_li[1:(dim_pak/9)]),
#                                              cumsum(df[df$country%in%c("Pakistan"),]$infections_li[(dim_pak/9+1):(2*dim_pak/9)]),
#                                              cumsum(df[df$country%in%c("Pakistan"),]$infections_li[(2*dim_pak/9+1):(3*dim_pak/9)]),
#                                              cumsum(df[df$country%in%c("Pakistan"),]$infections_li[(3*dim_pak/9+1):(4*dim_pak/9)]),
...
# 
# df[df$country%in%c("India"),]$UFR_ui=(tot_tot_cases_14320_ind+
#                                         c(cumsum(df[df$country%in%c("India"),]$infections_ui[1:(dim_ind/9)]),
#                                           cumsum(df[df$country%in%c("India"),]$infections_ui[(dim_ind/9+1):(2*dim_ind/9)]),
#                                           cumsum(df[df$country%in%c("India"),]$infections_ui[(2*dim_ind/9+1):(3*dim_ind/9)]),
#                                           cumsum(df[df$country%in%c("India"),]$infections_ui[(3*dim_ind/9+1):(4*dim_ind/9)]),
...
# 
# df[df$country%in%c("Bangladesh"),]$UFR_ui=(tot_tot_cases_14320_bang+
#                                              c(cumsum(df[df$country%in%c("Bangladesh"),]$infections_ui[1:(dim_bang/9)]),
#                                                cumsum(df[df$country%in%c("Bangladesh"),]$infections_ui[(dim_bang/9+1):(2*dim_bang/9)]),
#                                                cumsum(df[df$country%in%c("Bangladesh"),]$infections_ui[(2*dim_bang/9+1):(3*dim_bang/9)]),
#                                                cumsum(df[df$country%in%c("Bangladesh"),]$infections_ui[(3*dim_bang/9+1):(4*dim_bang/9)]),
...
# 
# 
# df[df$country%in%c("Pakistan"),]$UFR_ui=(tot_tot_cases_14320_pak+
#                                            c(cumsum(df[df$country%in%c("Pakistan"),]$infections_ui[1:(dim_pak/9)]),
#                                              cumsum(df[df$country%in%c("Pakistan"),]$infections_ui[(dim_pak/9+1):(2*dim_pak/9)]),
#                                              cumsum(df[df$country%in%c("Pakistan"),]$infections_ui[(2*dim_pak/9+1):(3*dim_pak/9)]),
#                                              cumsum(df[df$country%in%c("Pakistan"),]$infections_ui[(3*dim_pak/9+1):(4*dim_pak/9)]),
...
# 
# 
# df[df$country%in%c("Nepal"),]$UFR_med=(tot_tot_cases_14320_nep+
#                                          c(cumsum(df[df$country%in%c("Nepal"),]$infections_median[1:(dim_nep/9)]),
#                                            cumsum(df[df$country%in%c("Nepal"),]$infections_median[(dim_nep/9+1):(2*dim_nep/9)]),
#                                            cumsum(df[df$country%in%c("Nepal"),]$infections_median[(2*dim_nep/9+1):(3*dim_nep/9)]),
#                                            cumsum(df[df$country%in%c("Nepal"),]$infections_median[(3*dim_nep/9+1):(4*dim_nep/9)]),
...
# 
# 
# df[df$country%in%c("Nepal"),]$UFR_li=(tot_tot_cases_14320_nep+
#                                         c(cumsum(df[df$country%in%c("Nepal"),]$infections_li[1:(dim_nep/9)]),
#                                           cumsum(df[df$country%in%c("Nepal"),]$infections_li[(dim_nep/9+1):(2*dim_nep/9)]),
#                                           cumsum(df[df$country%in%c("Nepal"),]$infections_li[(2*dim_nep/9+1):(3*dim_nep/9)]),
#                                           cumsum(df[df$country%in%c("Nepal"),]$infections_li[(3*dim_nep/9+1):(4*dim_nep/9)]),
...
# 
# 
# df[df$country%in%c("Nepal"),]$UFR_ui=(tot_tot_cases_14320_nep+
#                                         c(cumsum(df[df$country%in%c("Nepal"),]$infections_ui[1:(dim_nep/9)]),
#                                           cumsum(df[df$country%in%c("Nepal"),]$infections_ui[(dim_nep/9+1):(2*dim_nep/9)]),
#                                           cumsum(df[df$country%in%c("Nepal"),]$infections_ui[(2*dim_nep/9+1):(3*dim_nep/9)]),
#                                           cumsum(df[df$country%in%c("Nepal"),]$infections_ui[(3*dim_nep/9+1):(4*dim_nep/9)]),
...
# 
# df[df$country%in%c("Sri_Lanka"),]$UFR_med=(tot_tot_cases_14320_sri+
#                                              c(cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_median[1:(dim_sri/9)]),
#                                                cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_median[(dim_sri/9+1):(2*dim_sri/9)]),
#                                                cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_median[(2*dim_sri/9+1):(3*dim_sri/9)]),
#                                                cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_median[(3*dim_sri/9+1):(4*dim_sri/9)]),
...
# 
# 
# df[df$country%in%c("Sri_Lanka"),]$UFR_li=(tot_tot_cases_14320_sri+
#                                             c(cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_li[1:(dim_sri/9)]),
#                                               cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_li[(dim_sri/9+1):(2*dim_sri/9)]),
#                                               cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_li[(2*dim_sri/9+1):(3*dim_sri/9)]),
#                                               cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_li[(3*dim_sri/9+1):(4*dim_sri/9)]),
...
# 
# df[df$country%in%c("Sri_Lanka"),]$UFR_ui=(tot_tot_cases_14320_sri+
#                                             c(cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_ui[1:(dim_sri/9)]),
#                                               cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_ui[(dim_sri/9+1):(2*dim_sri/9)]),
#                                               cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_ui[(2*dim_sri/9+1):(3*dim_sri/9)]),
#                                               cumsum(df[df$country%in%c("Sri_Lanka"),]$infections_ui[(3*dim_sri/9+1):(4*dim_sri/9)]),
...
# 
# date=which(x==as.Date("2020-12-31"))
# 
# df_b_m=df[(df$colour=="Original" & df$country=="India"),]$UFR_med
# df5_b_m=df[(df$colour=="Original" & df$country=="Bangladesh"),]$UFR_med
# df6_b_m=df[(df$colour=="Original" & df$country=="Pakistan"),]$UFR_med
# df7_b_m=df[(df$colour=="Original" & df$country=="Sri_Lanka"),]$UFR_med
# df8_b_m=df[(df$colour=="Original" & df$country=="Nepal"),]$UFR_med
# 
# 
# df_b_u=df[(df$colour=="Original" & df$country=="India"),]$UFR_ui
# df5_b_u=df[(df$colour=="Original" & df$country=="Bangladesh"),]$UFR_ui
# df6_b_u=df[(df$colour=="Original" & df$country=="Pakistan"),]$UFR_ui
# df7_b_u=df[(df$colour=="Original" & df$country=="Sri_Lanka"),]$UFR_ui
# df8_b_u=df[(df$colour=="Original" & df$country=="Nepal"),]$UFR_ui
# 
# df_b_l=df[(df$colour=="Original" & df$country=="India"),]$UFR_li
# df5_b_l=df[(df$colour=="Original" & df$country=="Bangladesh"),]$UFR_li
# df6_b_l=df[(df$colour=="Original" & df$country=="Pakistan"),]$UFR_li
# df7_b_l=df[(df$colour=="Original" & df$country=="Sri_Lanka"),]$UFR_li
# df8_b_l=df[(df$colour=="Original" & df$country=="Nepal"),]$UFR_li
# 
# 
# 
# tail(df_b_m[1:date],1)
# tail(df5_b_m[1:date],1)
# tail(df6_b_m[1:date],1)
# tail(df7_b_m[1:date],1)
# tail(df8_b_m[1:date],1)
# 
# tail(df_b_u[1:date],1)
# tail(df5_b_u[1:date],1)
# tail(df6_b_u[1:date],1)
# tail(df7_b_u[1:date],1)
# tail(df8_b_u[1:date],1)
# 
# 
# tail(df_b_l[1:date],1)
# tail(df5_b_l[1:date],1)
# tail(df6_b_l[1:date],1)
# tail(df7_b_l[1:date],1)
# tail(df8_b_l[1:date],1)
# 
# 
# 
# date_breaks <- "1 month"
